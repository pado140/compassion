...
class Member extends MY_Controller {
    ...
    * ********************************************************** */
    public function index() {
        ...
        $this->data['list'] = TRUE;
        $this->layout->title($this->lang->line('library') . ' ' . $this->lang->line('member') . ' | ' . CH);
    ...
    * ********************************************************** */
    public function add() {
        ...
        $this->layout->title($this->lang->line('library') . ' ' . $this->lang->line('non_member') . ' | ' . CH);
        $this->layout->view('member/index', $this->data);
    }
    ...
    * ********************************************************** */
    public function delete($id = null) {
        ...
        if ($this->book->delete('library_members', array('id' => $id))) {
            $this->book->update('enfants', array('is_library_member' => 0), array('user_id' => $member->user_id));
            
            $student = $this->book->get_single('enfants', array('user_id' => $member->user_id));
    ...
    public function add_to_library() {
        ...
        if ($user_id) {
            ...
            if (empty($member)) {
                ...
                $this->book->update('enfants', array('is_library_member' => 1), array('user_id' => $user_id));
                
                $enfant = $this->book->get_single('enfants', array('user_id' => $member->user_id));
                create_log('Has been added a Library Member : '.$enfant->nom);
