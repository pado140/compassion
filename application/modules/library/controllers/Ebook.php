...
class Ebook extends MY_Controller {
    ...
    public function index() {
        ...
        $this->data['ebooks'] = $this->ebook->get_ebook_list();            
       
        
        $this->data['list'] = TRUE;
        $this->layout->title($this->lang->line('manage') .' ' .$this->lang->line('e_book'). ' | ' . CH);
    ...
    public function add() {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($insert_id) {                    
                    ...
                    create_log('Has been added ebook : '. $data['name']);
                                        
                    success($this->lang->line('insert_success'));
                    redirect('library/ebook/index');
                } else {
                    error($this->lang->line('insert_failed'));
                    redirect('library/ebook/add');
                }
            } else {
        ...
        $this->data['add'] = TRUE;
        $this->layout->title($this->lang->line('add'). ' ' . $this->lang->line('e_book'). ' | ' . CH);
        $this->layout->view('ebook/index', $this->data);
    }
    ...
    public function edit($id = null) {       
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($updated) {
                    ...
                    create_log('Has been updated ebook : '. $data['name'].' ;');  
        ...
        $this->data['edit'] = TRUE;       
        $this->layout->title($this->lang->line('edit'). ' ' . $this->lang->line('e_book'). ' | ' . CH);
        $this->layout->view('ebook/index', $this->data);
    ...
    public function view( $ebook_id = null ){
        ...
        $this->layout->title($this->lang->line('view'). ' ' . $this->lang->line('e_book'). ' | ' . CH);
        $this->layout->view('ebook/index', $this->data);
    }
    ...
    private function _upload_file_name(){
        ...
        if ($ebook != "") {
            if ($ebook_type == 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' || 
                ...
                $ebook_path = 'ebook-'.time() . '-comp.' . $extension;
    ...
    private function _upload_cover_image() {
        ...
        if ($image != "") {
            if ($image_type == 'image/jpeg' || $image_type == 'image/pjpeg' ||
                ...
                $extension = strtolower($file_type[count($file_type) - 1]);
                $image_path = 'cover-image-' . time() . '-comp.' . $extension;
