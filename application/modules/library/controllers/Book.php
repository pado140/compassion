...
class Book extends MY_Controller {
    ...
    * ********************************************************** */
    public function index() {
        ...
        $this->data['list'] = TRUE;
        $this->layout->title($this->lang->line('manage_book') . ' | ' . CH);
    ...
    * ********************************************************** */
    public function add() {
        ...
        $this->data['roles'] = $this->book->get_list('roles', array('status' => 1), '', '', '', 'id', 'ASC');
        $this->data['categories'] = $this->category->get_list('category', array('status' => 1), '', '', '', 'id', 'ASC');
        $this->data['custom_id'] = $this->book->get_custom_id('books', 'BK');
        
        $this->data['add'] = TRUE;
        $this->layout->title($this->lang->line('add') . ' ' . $this->lang->line('book') . ' | ' . CH);
        $this->layout->view('book/index', $this->data);
    }
    ...
    public function edit($id = null) {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                $this->data['book'] = $this->book->get_single_book($this->input->post('id'));
            }
        }

        if ($id) {
            $this->data['book'] = $this->book->get_single_book($id);
        ...
        $this->data['edit'] = TRUE;
        $this->layout->title($this->lang->line('edit') . ' ' . $this->lang->line('book') . ' | ' . CH);
        $this->layout->view('book/index', $this->data);
    }
    ...
    private function _upload_cover() {
        ...
        if ($cover != "") {
            if ($cover_type == 'image/jpeg' || $cover_type == 'image/pjpeg' ||
                ...
                $cover_path = 'book-' . time() . '-comp.' . $extension;
